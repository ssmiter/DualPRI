...
class MambaChunkScanCombinedFn(torch.autograd.Function):
    ...
    def forward(ctx, x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None, seq_idx=None, cu_seqlens=None, dt_softplus=False, dt_limit=(0.0, float("inf")), return_final_states=False, return_varlen_states=False, return_states=False):
        ...
        out, out_x, dt_out, dA_cumsum, states, final_states, *rest \
            = _mamba_chunk_scan_combined_fwd(x, dt, A, B, C,
                                             chunk_size, D=D,
                                             z=z,
                                             dt_bias=dt_bias,
...
def mamba_chunk_scan_combined(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None,
                              seq_idx=None, cu_seqlens=None, dt_softplus=False, dt_limit=(0.0, float("inf")),
                              return_final_states=False, return_varlen_states=False, return_states=False):
    """
    Argument:
        x: (batch, seqlen, nheads, headdim)
        ...
        return_states: Whether to return intermediate states for each chunk
    Return:
        out: (batch, seqlen, nheads, headdim)
        [optional] final_states: (batch, nheads, headdim, dstate) if return_final_states is True
        [optional] states: (batch, nchunks, nheads, headdim, dstate) if return_states is True
        [optional] varlen_states: if return_varlen_states is True
    """
    return MambaChunkScanCombinedFn.apply(x, dt, A, B, C, chunk_size, D, z, dt_bias, initial_states, seq_idx, cu_seqlens, dt_softplus, dt_limit, return_final_states, return_varlen_states, return_states)
...
def mamba_conv1d_scan_ref(xBC, conv1d_weight, conv1d_bias, dt, A, chunk_size, D=None, z=None,
    ...
    out = ssd_selective_scan(x, dt.to(x.dtype), A, B, C, D=D.float(), z=z, dt_bias=dt_bias, dt_softplus=dt_softplus,
                             dt_limit=dt_limit)
...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    @staticmethod
    @custom_fwd
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None, seq_idx=None,
                dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu",
        ...
        ctx.save_for_backward(zxbcdt, conv1d_weight, conv1d_bias,
                              out_x, A, D, dt_bias, initial_states, seq_idx, rmsnorm_weight, rstd, outproj_weight,
                              outproj_bias)
        ctx.dt_limit = dt_limit
        ctx.return_final_states = return_final_states
    ...
    def backward(ctx, dout, *args):
        ...
        dzx0, dz, dxBC_given, ddt_given = torch.split(dzxbcdt,
                                                      [2 * d_nonssm, dim, dim + 2 * ctx.ngroups * dstate, nheads],
                                                      dim=-1)
        ...
        if rmsnorm_weight is None:
            ...
            dx, ddt, dA, dB, dC, dD, dz, ddt_bias, dinitial_states, *rest = _mamba_chunk_scan_combined_bwd(
                ...
                ddt=ddt_given, dB=dB, dC=dC, dz=dz, recompute_output=recompute_output
            )
            ...
            dout, drmsnorm_weight, _, dz, *rest = _layer_norm_bwd(dy_rms, x_rms, rmsnorm_weight, None, ctx.rmsnorm_eps,
                                                                  ...
                                                                  is_rms_norm=True, recompute_output=recompute_output,
                                                                  dz=dz,
        ...
        dxBC_given, dweight, dbias, *_ = causal_conv1d_cuda.causal_conv1d_bwd(
            ...
            rearrange(dxBC, "b s d -> b d s"), seq_idx, None, None, dxBC_given, False,
            ctx.activation in ["silu", "swish"]
        )
...
def mamba_split_conv1d_scan_combined(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None,
                                     seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False,
                                     activation="silu", rmsnorm_weight=None, rmsnorm_eps=1e-6, outproj_weight=None,
...
def mamba_split_conv1d_scan_ref(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size,
                                dt_limit=(0.0, float("inf")), activation="silu", rmsnorm_weight=None, rmsnorm_eps=1e-6,
